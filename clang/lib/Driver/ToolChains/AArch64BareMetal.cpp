...
std::string AArch64BareMetalToolChain::getThreadModel() const {
  return "single";
}
...
void Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const toolchains::AArch64BareMetalToolChain &TC =
      static_cast<const toolchains::AArch64BareMetalToolChain &>(
          getToolChain());
  bool UsesLLD = Args.getLastArgValue(options::OPT_fuse_ld_EQ)
                     .endswith_insensitive("lld") ||
                 LinkerName == "ld.lld";
  ...
  if (UseStartfiles) {
    ...
    CmdArgs.push_back(
        Args.MakeArgString(TC.getStdLibDir() + "/cpu-init/rdimon-aem-el3.o"));
  }
  ...
  if (!Args.hasArg(options::OPT_nostdlib, options::OPT_nodefaultlibs)) {
    ...
    // but still want some of them to be added explicitely from the command
    // line.
  ...
  C.addCommand(std::make_unique<Command>(JA, *this, ResponseFileSupport::None(),
                                         Linker, CmdArgs, Inputs));

  // TODO: If the user explicitely asked for capsizefix, then invoke it.
}
...
void Assembler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const char *Assembler =
      Args.MakeArgString(TC.GetProgramPath(AssemblerName.c_str()));
  C.addCommand(std::make_unique<Command>(JA, *this, ResponseFileSupport::None(),

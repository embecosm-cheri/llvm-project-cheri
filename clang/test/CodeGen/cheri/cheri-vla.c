...
void foo(void) {
  ...
  // CHECK: [[VAR0:%.+]] = call ptr addrspace(200) @llvm.stacksave.p200()
  // CHECK: [[VLA:%.+]] = alloca i8, i64 [[I_05]], align 1
  // CHECK: [[SUB:%.+]] = add nsw i64 [[I_05]], -1
  // CHECK: [[ARRAYIDX:%.+]] = getelementptr inbounds i8, ptr addrspace(200) [[VLA]], i64 [[SUB]]
  // CHECK: store i8 0, ptr addrspace(200) [[ARRAYIDX]], align 1, !tbaa !{{[0-9]+}}
  // CHECK: call void @test(ptr addrspace(200) noundef nonnull [[VLA]])
  // CHECK: call void @llvm.stackrestore.p200(ptr addrspace(200) [[VAR0]])

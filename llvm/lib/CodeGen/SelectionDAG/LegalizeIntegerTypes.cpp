...
static SDValue ExpandExtIntRes_DIVREM(const TargetLowering &TLI,
  ...
  int Bits = N->getOperand(0)
                 ...
                 ->getIntegerBitWidth();
  Entry.Node = DAG.getConstant(Bits, DL, TLI.getPointerRangeTy(DAG.getDataLayout()));
  ...
  SDValue Callee = DAG.getExternalSymbol(TLI.getLibcallName(LC),
                                         TLI.getPointerTy(DAG.getDataLayout(), DAG.getDataLayout().getGlobalsAddressSpace()));
...
void DAGTypeLegalizer::ExpandIntRes_XMULO(SDNode *N,
  ...
  Entry.Node = Temp;
  Entry.Ty =
      PtrTy->getPointerTo(DAG.getDataLayout().getDefaultGlobalsAddressSpace());
  Entry.IsSExt = true;
  Entry.IsZExt = false;
  Args.push_back(Entry);

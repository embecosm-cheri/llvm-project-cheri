...
static cl::opt<unsigned>
    LowBinThreshold("set-bounds-stat-threshold", cl::Hidden,
                    cl::desc("Threshold for considering a bound value in the "
                             "low bin (default = 512)"),
                    cl::init(512));
...
public:
  ...
  void gatherConstantOffsetAccesses(Function &F, const DataLayout &DL,
    ...
    for (auto I = F.begin(), E = F.end(); I != E; ++I)
      for (Instruction &Inst : *I) {
        ...
        // Get the alloca size
        Value *ASz = AllocaBase->isArrayAllocation()
                         ? AllocaBase->getArraySize()
  ...
  void sinkSetLen(Instruction *SetLen, DominatorTree &DT, LoopInfo &LI) {
    ...
    for (DomTreeNode::iterator I = DTN->begin(), E = DTN->end();
      ...
      // tree.
      if (IsAcceptableTarget(Users, SetLen->getParent(), Candidate, DT, LI,
                             SetLen))
  ...
  bool isSmallStackFunction(const DataLayout &DL) {
    ...
    for (AllocaInst *AI : Allocas) {
      ...
      Value *ASz = AI->isArrayAllocation()
                       ? AI->getArraySize()
                       : ASz = ConstantInt::get(Type::getInt64Ty(C), 1);
  ...
  bool runOnFunction(Function &F) override {
    ...
    Type *SizeTy = Type::getIntNTy(M->getContext(), DL.getIndexSizeInBits(200));
    ...
    for (AllocaInst *AI : Allocas) {
      ...
      if (ConstantInt *CI = dyn_cast<ConstantInt>(AI->getArraySize())) {
        ...
        Align Alignment = Req.second ? Align(1UL << Req.second) : Align(1);
        TotalSize = Req.first;
      ...
      if (Alloca) {
        ...
        Value *NAlloca = B.CreateBitCast(Alloca, AI->getType());
      ...
      Value *NAlloca = B.CreateBitCast(
          OldAlloca, SetLenFun->getFunctionType()->getParamType(0));
...
INITIALIZE_PASS_END(AArch64Sandbox, "aarch64-sandbox", "AArch64 sandbox setup",
                    false, false)
